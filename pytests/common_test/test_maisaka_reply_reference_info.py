...
class _FakeLLMServiceClient:
    ...
    async def generate_response_with_context(
        self,
        *,
        context_factory: Callable[..., list[Any]],
        options: Any = None,
    ) -> LLMResponseResult:
        del options
        assert await _call_message_factory(context_factory, object())
...
def _build_reply_tool_ctx(chat_history: list[Any]) -> BuiltinToolRuntimeContext:
    ...
    return BuiltinToolRuntimeContext(
        engine=cast("MaisakaReasoningEngine", cast(object, engine)),
        runtime=cast("MaisakaHeartFlowChatting", cast(object, runtime)),
    )
...
async def test_replyer_hooks_receive_reference_info(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(replyer_module, "LLMServiceClient", _FakeLLMServiceClient)
